...
public class Sprite extends TextureRegion
{
  ...
  {
    paramBatch.draw(this.texture, getVertices(), 0, 20);
  }
  ...
  {
    int i = NumberUtils.floatToIntColor(this.vertices[2]);
    float f = NumberUtils.intToFloatColor((int)(255.0F * paramFloat) << 24 | i & 0xFFFFFF);
    this.vertices[2] = f;
    this.vertices[7] = f;
    this.vertices[12] = f;
    this.vertices[17] = f;
  ...
  {
    setX(paramFloat - this.width / 2.0F);
  }

  public void setCenterY(float paramFloat)
  {
    setY(paramFloat - this.height / 2.0F);
  }
