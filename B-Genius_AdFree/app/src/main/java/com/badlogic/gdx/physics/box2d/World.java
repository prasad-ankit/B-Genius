  ...
  implements Disposable
{
  ...
  protected final Pool freeBodies = new World.1(this, 100, 200);
  private final Array freeContacts = new Array();
  protected final Pool freeFixtures = new World.2(this, 100, 200);
  ...
  private boolean contactFilter(long paramLong1, long paramLong2)
  {
    ...
    if (this.contactFilter != null)
      bool1 = this.contactFilter.shouldCollide((Fixture)this.fixtures.get(paramLong1), (Fixture)this.fixtures.get(paramLong2));
    Filter localFilter1;
    Filter localFilter2;
    while (true)
    {
  ...
  private long createProperJoint(JointDef paramJointDef)
  {
    ...
    {
      MotorJointDef localMotorJointDef = (MotorJointDef)paramJointDef;
      return jniCreateMotorJoint(this.addr, localMotorJointDef.bodyA.addr, localMotorJointDef.bodyB.addr, localMotorJointDef.collideConnected, localMotorJointDef.linearOffset.x, localMotorJointDef.linearOffset.y, localMotorJointDef.angularOffset, localMotorJointDef.maxForce, localMotorJointDef.maxTorque, localMotorJointDef.correctionFactor);
    }
    ...
    if (paramJointDef.type == JointDef.JointType.WeldJoint)
    {
      ...
      return jniCreateWeldJoint(this.addr, localWeldJointDef.bodyA.addr, localWeldJointDef.bodyB.addr, localWeldJointDef.collideConnected, localWeldJointDef.localAnchorA.x, localWeldJointDef.localAnchorA.y, localWeldJointDef.localAnchorB.x, localWeldJointDef.localAnchorB.y, localWeldJointDef.referenceAngle, localWeldJointDef.frequencyHz, localWeldJointDef.dampingRatio);
    }
  ...
  public final Joint createJoint(JointDef paramJointDef)
  {
    ...
    if (paramJointDef.type == JointDef.JointType.GearJoint)
      localObject = new GearJoint(this, l, ((GearJointDef)paramJointDef).joint1, ((GearJointDef)paramJointDef).joint2);
    if (paramJointDef.type == JointDef.JointType.MotorJoint)
      localObject = new MotorJoint(this, l);
    if (paramJointDef.type == JointDef.JointType.MouseJoint)
    ...
    return (Joint)localObject;
  }

  final void deactivateBody(Body paramBody)
  {
    jniDeactivateBody(this.addr, paramBody.addr);
  }

  public final void destroyBody(Body paramBody)
  {
    Array localArray1 = paramBody.getJointList();
    while (localArray1.size > 0)
    ...
    Array localArray2 = paramBody.getFixtureList();
    while (localArray2.size > 0)
    {
      Fixture localFixture = (Fixture)localArray2.removeIndex(0);
      ((Fixture)this.fixtures.remove(localFixture.addr)).setUserData(null);
      this.freeFixtures.free(localFixture);
    }
    this.freeBodies.free(paramBody);
  }

  final void destroyFixture(Body paramBody, Fixture paramFixture)
  {
    jniDestroyFixture(this.addr, paramBody.addr, paramFixture.addr);
  }
  ...
  public final void getBodies(Array paramArray)
  {
    paramArray.clear();
    paramArray.ensureCapacity(this.bodies.size);
    LongMap.Values localValues = this.bodies.values();
    while (localValues.hasNext())
      paramArray.add(localValues.next());
  }
  ...
  public final void getFixtures(Array paramArray)
  {
    paramArray.clear();
    paramArray.ensureCapacity(this.fixtures.size);
    LongMap.Values localValues = this.fixtures.values();
    while (localValues.hasNext())
      paramArray.add(localValues.next());
  }
  ...
  public final void getJoints(Array paramArray)
  {
    paramArray.clear();
    paramArray.ensureCapacity(this.joints.size);
    LongMap.Values localValues = this.joints.values();
    while (localValues.hasNext())
      paramArray.add(localValues.next());
  }
