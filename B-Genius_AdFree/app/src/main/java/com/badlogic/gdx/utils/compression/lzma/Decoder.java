...
public class Decoder
{
  ...
  public boolean Code(InputStream paramInputStream, OutputStream paramOutputStream, long paramLong)
  {
    ...
    if ((paramLong < 0L) || (l1 < paramLong))
    {
      ...
      if (this.m_RangeDecoder.DecodeBit(this.m_IsMatchDecoders, i2 + (i1 << 4)) == 0)
      {
        ...
        for (b = localDecoder2.DecodeWithMatchByte(this.m_RangeDecoder, this.m_OutWindow.GetByte(m)); ; b = localDecoder2.DecodeNormal(this.m_RangeDecoder))
        {
    ...
    while (true)
    {
      ...
      while (true)
      {
        if ((m < l1) && (m < this.m_DictionarySizeCheck))
          break label577;
        return false;
        int i15;
      ...
      label577: this.m_OutWindow.CopyBlock(m, i3);
      long l2 = l1 + i3;
  ...
  public boolean SetDecoderProperties(byte[] paramArrayOfByte)
  {
    ...
    do
    {
      ...
      while (i1 < 4)
      {
        i2 += ((0xFF & paramArrayOfByte[(i1 + 1)]) << (i1 << 3));
        i1++;
      }
    }
